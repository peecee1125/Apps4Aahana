...
function initState(questions) {
  return {
    questions: shuffle(questions).slice(0, PER_ROUND).map(q => ({ ...q, choices: shuffle(q.choices) })),
    index:      0,
    score:      0,
    streak:     0,
    bestStreak: 0,
...
export default function QuizScreen({ subject, testIdx, onComplete, onBack }) {
  ...
  const [state, dispatch] = useReducer(reducer, test?.questions ?? [], initState)
  ...
  useEffect(() => {
    if (isDone) onComplete({ score: state.score, bestStreak: state.bestStreak })
  ...
  const answer = useCallback((choice) => {
    ...
    if (choice === q.answer) playCorrect(); else playWrong()
  ...
  function btnBg(choice) {
    ...
    if (choice === answered.selected && choice !== answered.correct) return '#dc2626'
  ...
  return (
    ...
    >
      <NavHeader
        ...
        onBack={() => { playTap(); clearTimeout(timerRef.current); onBack() }}
      />
