...
import { SUBJECTS, loadHighScore, saveHighScore } from '../data/registry'
...
export default function ResultsScreen({ subject, testIdx, results, onPlayAgain, onHome, onSubjectMenu }) {
  ...
  return (
    ...
    >
      ...
      <NavHeader title="Round Complete! 🎓" onBack={onHome} backLabel="🏠 Home" />
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-5 px-8 min-h-0">
        ...
        >
          {[1,2,3].map(s => (
            <span key={s} className="text-5xl drop-shadow-lg" style={{ opacity: stars >= s ? 1 : 0.18 }}>⭐</span>
          ))}
        </motion.div>
        ...
        >
          <div className="text-white/60 font-bold text-lg">{subj?.emoji} {test?.label}</div>
          <div className="font-black text-8xl leading-none mt-1" style={{ color:'#fbbf24' }}>
            {score}<span className="text-3xl text-white/40">/100</span>
          </div>
          <div className="text-white/50 text-base mt-1">{score / 10} of 10 correct</div>
        </motion.div>
          ...
          >
            {badges.map(b => (
              <div key={b.label}
                ...
                style={{ background:'rgba(251,191,36,0.15)', border:'1.5px solid #fbbf24' }}
              >
        ...
        >
          <motion.button whileTap={{ scale:0.93 }} onClick={onPlayAgain}
            className="px-8 py-4 rounded-2xl font-extrabold text-white text-lg shadow-xl"
            style={{ background: subj?.bg ?? 'linear-gradient(135deg,#7c3aed,#4f46e5)' }}
          >🔄 Play Again</motion.button>
          <motion.button whileTap={{ scale:0.93 }} onClick={onSubjectMenu}
            className="px-8 py-4 rounded-2xl font-extrabold text-white text-lg shadow-xl"
            style={{ background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)' }}
          >📋 All Tests</motion.button>
        </motion.div>
      </div>
    </motion.div>
  )
